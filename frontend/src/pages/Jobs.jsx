...
const fmtNum = (v) =>
  v == null || Number.isNaN(v) ? "–" : nf.format(Math.round(v));
const fmtMoney = (v) =>
...
const fmtMs = (v) =>
  v == null || Number.isNaN(v) ? "–" : `${Math.round(v)} ms`;
const fmtSec = (v) =>
  v == null || Number.isNaN(v) ? "–" : `${(Number(v) / 1000).toFixed(1)} s`;
const safeGet = (obj, path, d = undefined) =>
  path
    .split(".")
    .reduce(
...
function normalizeJobMetrics(jobData) {
  ...
  const perLang = targets.map((t) => {
    ...
    return {
      ...
      status: t?.status || "—",
      tokens_in:
        Number(m.tokens_in ?? m.input_tokens ?? m.src_tokens ?? 0) || 0,
      tokens_out:
        Number(m.tokens_out ?? m.output_tokens ?? m.tgt_tokens ?? 0) || 0,
  ...
  const targets_failed = targets.filter(
    (t) => t.status === "failed"
  ).length;

  const mergedSummary = {
    tokens_in:
      Number(jobMetrics.tokens_in ?? jobMetrics.input_tokens ?? tokens_in) ||
      tokens_in,
    tokens_out:
      Number(
        jobMetrics.tokens_out ??
          jobMetrics.output_tokens ??
          tokens_out
      ) || tokens_out,
    cost_usd:
      Number(jobMetrics.cost_usd ?? jobMetrics.cost ?? cost_usd) ||
      cost_usd,
    latency_ms_avg:
      Number(
        jobMetrics.latency_ms_avg ??
          jobMetrics.latency ??
          latency_ms_avg
      ) || latency_ms_avg,
    duration_ms_sum:
      Number(
        jobMetrics.duration_ms_sum ??
          jobMetrics.duration ??
          duration_ms_sum
      ) || duration_ms_sum,
    targets_done,
...
export default function Jobs() {
  ...
  const showToast = (msg, tone = "success") =>
    setToast({ show: true, tone, msg });
  const closeToast = () =>
    setToast((tState) => ({ ...tState, show: false }));
  ...
  const pollJobUntilDone = async (
    ...
    while (tries < maxTries) {
      ...
      try {
        ...
        const anyRunning = targets.some((t) =>
          ["processing", "queued"].includes(t.status)
        );
        const jobDone =
          data.status === "done" ||
          (!anyRunning && ["mixed", "failed"].includes(data.status));
    ...
    showToast(
      t(
        ...
        {
          defaultValue:
            "Job {{id}} ainda em processamento. Atualize mais tarde.",
        }
      ),
      "info"
    );
  ...
  const onSubmit = async (e) => {
    ...
    const cleanedTargets = Array.from(
      new Set(targetLangs.filter((t) => t && t !== sourceLang))
    );
    if (cleanedTargets.length === 0) {
      alert(
        t("jobs.needDifferentTarget", {
          defaultValue:
            "Selecione pelo menos 1 destino diferente do idioma de origem.",
        })
      );
      return;
    }
    ...
    cleanedTargets.forEach((tgt) =>
      form.append("target_langs", tgt)
    );
    ...
    try {
      ...
      alert(
        t("jobs.couldNotCreate", {
          defaultValue:
            "Não foi possível criar o job. Verifique o arquivo e tente novamente.",
        })
      );
    } finally {
      setUploading(false);
  ...
  const onDownload = async (job) => {
    ...
    try {
      ...
      a.download =
        targets.length > 1
          ? `job_${job.id}_outputs.zip`
          : job.title || `output_${job.id}.docx`;
      ...
      alert(
        t("jobs.downloadNotReady", {
          defaultValue:
            "Saída ainda não está pronta ou ocorreu um erro no download.",
        })
      );
    }
  };
  ...
  const disabledUpload =
    !file ||
    uploading ||
    targetLangs.filter((t) => t !== sourceLang).length === 0;
        ...
        <div className="flex flex-wrap items-center gap-4 md:gap-6">
          ...
          <div className="w-[220px] self-center">
            ...
            <select
              value={glossaryId ?? ""}
              onChange={(e) =>
                setGlossaryId(Number(e.target.value) || null)
              }
              className="h-10 w-full rounded-xl border border-black/10 dark:border-white/10 bg-white dark:bg-neutral-900 px-3"
            >
              <option value="">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4 text-sm">
              <div className="rounded-xl border border-black/10 dark:border-white/10 p-3">
                ...
                <div className="font-medium">
                  {fmtNum(
                    metricsJob.metrics?.tokens_in
                  )}
                </div>
              </div>
              <div className="rounded-xl border border-black/10 dark:border-white/10 p-3">
                <div className="text-neutral-500">
                  {t("metrics.tokens_out")}
                </div>
                <div className="font-medium">
                  {fmtNum(
                    metricsJob.metrics?.tokens_out
                  )}
                </div>
              </div>
              <div className="rounded-xl border border-black/10 dark:border-white/10 p-3">
                <div className="text-neutral-500">
                  {t("metrics.cost_total")}
                </div>
                <div className="font-medium">
                  {fmtMoney(
                    metricsJob.metrics?.cost_usd
                  )}
                </div>
              </div>
              <div className="rounded-xl border border-black/10 dark:border-white/10 p-3">
                <div className="text-neutral-500">
                  {t("metrics.latency_avg")}
                </div>
                <div className="font-medium">
                  {fmtMs(
                    metricsJob.metrics?.latency_ms_avg
                  )}
                </div>
              </div>
              <div className="rounded-xl border border-black/10 dark:border-white/10 p-3">
                <div className="text-neutral-500">
                  {t("metrics.time_total")}
                </div>
                <div className="font-medium">
                  {fmtSec(
                    metricsJob.metrics?.duration_ms_sum
                  )}
                </div>
              </div>
              <div className="rounded-xl border border-black/10 dark:border-white/10 p-3">
                <div className="text-neutral-500">
                  {t("metrics.destinations")}
                </div>
                <div className="font-medium">
                  {(metricsJob.metrics?.targets_done ??
                    0)}{" "}
                  {t("jobs.okShort", {
                    defaultValue: "ok",
                  })}{" "}
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-black/[0.03] dark:bg-white/[0.06] text-left">
                  <tr>
                    ...
                    <th className="px-3 py-2">
                      {t("metrics.tokens_in_col")}
                    </th>
                    <th className="px-3 py-2">
                      {t("metrics.tokens_out_col")}
                    </th>
                    <th className="px-3 py-2">
                      {t("metrics.cost_col")}
                    </th>
                    <th className="px-3 py-2">
                      {t("metrics.latency_col")}
                    </th>
                    <th className="px-3 py-2">
                      {t("metrics.duration_col")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {Array.isArray(metricsJob.perLang) &&
                  metricsJob.perLang.length > 0 ? (
                    metricsJob.perLang.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 tabular-nums">
                          {fmtNum(row.tokens_in)}
                        </td>
                        <td className="px-3 py-2 tabular-nums">
                          {fmtNum(row.tokens_out)}
                        </td>
                        <td className="px-3 py-2 tabular-nums">
